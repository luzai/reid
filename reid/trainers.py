...
class TriTrainer(object):
    def __init__(self, model, criterion, logs_at='work/vis', dbg=True, args=None, dop_info=None, **kwargs):
        ...
        if dbg:
            ...
            self.writer = SummaryWriter2(logs_at + '/bak')
    
    def train(self, epoch, data_loader, optimizer, print_freq=5, schedule=None, ft=False):
        batch_time = AverageMeter()
        data_time = AverageMeter()
        losses = AverageMeter()
        precisions = AverageMeter()
        ...
        for i, inputs in enumerate(data_loader):
            ...
            gl_features, features = self.model(input_imgs, ft=ft)  # todo
            # mid_feas.append(features)
            losst = 0
            for gl_fea in gl_features:
                losst += self.criterion(gl_fea, targets, dbg=False)[0] / 2.
            if self.args.loss != 'tri_adv':
                losstt, prect, dist_tri = self.criterion(features, targets, dbg=False, temp=1/5)
            else:
                losstt, prect, xa, xp, xn = self.criterion(features, targets, dbg=False, temp=1/5)
            losst += losstt
            ...
            # 7. middle
                ...
                optimizer.zero_grad()
                from exps.cfgs.single_ohnm import amp_handle
                with amp_handle.scale_loss(losst, optimizer) as scaled_loss:
