...
def worker_func(model_module, model_kwargs, checkpoint, dataset, data_func,
                gpu_id, idx_queue, result_queue):
    torch.cuda.set_device(gpu_id)
    model = model_module(**model_kwargs)
...
def parallel_test(model_module,
                  model_kwargs,
                  checkpoint,
                  dataset,
                  data_func,
    ...
    workers = [
        ctx.Process(
            target=worker_func,
            args=(model_module, model_kwargs, checkpoint, dataset, data_func,
