...
cfgs = [
    ...
    # ),
    edict(
        logs_dir='cu03det.xent',
        dataset='cu03det',
        batch_size=128, num_instances=4, gpu=range(1), num_classes=128,
        dropout=0, loss='xent', xent_smooth=True,
        gpu_range=range(4),
    ),
    # edict(
    #     logs_dir='cu03det.trixent',
    #     dataset='cu03det',
    #     batch_size=128, num_instances=4, gpu=range(1), num_classes=128,
    #     dropout=0, loss='tri_xent', xent_smooth=True,
    # )
]
...
# )
#
# for scale in [1., 1e-3, 1e-5, ]:
#     cfg_t = copy.deepcopy(cfg)
#     cfg_t.push_scale = scale
#     cfg_t.logs_dir = f'{cfg_t.logs_dir}.{scale:.0e}'
#     cfgs.append(cfg_t)
